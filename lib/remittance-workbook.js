...
function salesBlock(txns, items) {
  ...
  // their own line, and counting them would inflate every basket by one. The
  // Pleasant Hill sheet reads 20 over a period whose transaction log carries an
  // Item Count of 34 — the 14 fee lines are the difference — so this is the
  // definition finance uses, and the transaction log's own Item Count is not.
  const lineItems = items.filter(i => String(i["Item Type"] || "") !== FEE_ITEM);
  return {
...
function summarize({ txns, items, fees }) {
  ...
  const sales   = salesBlock(pay, items.filter(isPayment));
  const refunds = salesBlock(ref, items.filter(isRefund));
...
// letters are derived from the column list rather than typed. Adding a column
// to the export then moves the formulas with it instead of silently pointing
// them one column left.
const TXN_COL  = Object.fromEntries(TXN_COLUMNS.map((n, i) => [n, colName(i)]));
const ITEM_COL = Object.fromEntries(ITEM_COLUMNS.map((n, i) => [n, colName(i)]));
...
function summarySheet(org, period, s) {
  ...
  const T = (name) => `'${TXN_SHEET}'!$${TXN_COL[name]}$2:$${TXN_COL[name]}$${txnRows + 1}`;
  const sumIf = (col, type) => `IFERROR(SUMIFS(${T(col)}, ${T("Type")}, "${type}"), 0)`;
  const countIf = (col, type) => `IFERROR(COUNTIFS(${T("Type")}, "${type}", ${T(col)}, ">0"), 0)`;
  // Items are counted off the ITEMS log with the fee lines excluded, which is
  // the figure the finance sheet carries — see salesBlock.
  const itemRows = s.itemRowCount || 0;
  const I = (name) => `'${ITEM_SHEET}'!$${ITEM_COL[name]}$2:$${ITEM_COL[name]}$${itemRows + 1}`;
  const itemsOf = (type) =>
    `IFERROR(COUNTIFS(${I("Type")}, "${type}", ${I("Item Type")}, "<>${FEE_ITEM}"), 0)`;
  ...
  f = zebra(); line("Items Sold", { v: s.sales.itemCount, f: itemsOf("payment") }, "int", f);
  ...
  f = zebra(); line("Items Refunded", { v: s.refunds.itemCount, f: itemsOf("refund") }, "int", f);
...
function generate({ org, period, txns, items, fees }) {
  ...
  const sheets = [
    ...
    dataSheet(TXN_SHEET,  TXN_COLUMNS,  txns),
    dataSheet(ITEM_SHEET, ITEM_COLUMNS, items),
  ];
...
module.exports = {
  ...
  TENDERS, TXN_COLUMNS, ITEM_COLUMNS, TXN_SHEET, ITEM_SHEET, TXN_COL, ITEM_COL, FEE_ITEM,
};
